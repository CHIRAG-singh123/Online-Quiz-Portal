...
from .forms import SemesterFilterForm  # Import the SemesterFilterForm
from student.models import Student  # Import your Student model
from quiz.models import Course  # Import your Course model
from .forms import SemesterFilterForm  # Import the SemesterFilterForm
            ...
            if csv_file.name.endswith('.csv'):
                ...
                for row in csv.reader(io_string, delimiter=',', quotechar="|"):
                    _, created = Course.objects.get_or_create(
                        course_name=row[0],  # Assuming the first column is courseID
                        question_number=row[1],
                        total_marks=row[2],
                        sem=row[3],
                        
                    )
                return redirect('admin-view-course')  # Redirect to a success page
            else:
                return render(request, 'quiz/admin-csv-course.html', {'error_message': 'Please upload a valid CSV file.'})
    else:
        form = CSVUploadForm()
    return render(request, 'quiz/admin-csv-course.html', {'form': form})
#csv for question
@login_required(login_url='adminlogin')
def admin_csv_question(request):
    if request.method == 'POST':
        form = CSVUploadForm(request.POST, request.FILES)
        if form.is_valid():
            csv_file = form.cleaned_data['csv_file']
            ...
            if csv_file.name.endswith('.csv'):
                ...
                for row in csv.reader(io_string, delimiter=',', quotechar="|"):
                    _, created = Question.objects.get_or_create(
                        course_id=row[0],  # Assuming the first column is courseID
                        marks=row[1],
                        question=row[2],
                        option1=row[3],
                        option2=row[4],
                ...
                return redirect('admin-view-question')  # Redirect to a success page
            else:
                return render(request, 'quiz/admin-csv-question.html', {'error_message': 'Please upload a valid CSV file.'})
    else:
        form = CSVUploadForm()
    return render(request, 'quiz/admin-csv-question.html', {'form': form})
...
def change_password_admin(request):
    ...
    return render(request, 'quiz/change_password_admin.html', {
        'form': form
    })
...
def admin_view_student_view(request):
    if request.method == 'POST':
        form = SemesterFilterForm(request.POST)
        if form.is_valid():
            selected_semester = form.cleaned_data['sem']  # Correct field name
            students = SMODEL.Student.objects.filter(sem=selected_semester)  # Use the correct field name
        else:
            form = SemesterFilterForm()
    ...
    return render(request, 'quiz/admin_view_student.html', {'students': students, 'form': form})
...
def admin_view_course_view(request):
    if request.method == 'POST':
        form = SemesterFilterForm(request.POST)
        if form.is_valid():
            selected_semester = form.cleaned_data['sem']
            courses = Course.objects.filter(sem=selected_semester)
    else:
        form = SemesterFilterForm()
        courses = Course.objects.all()

    return render(request, 'quiz/admin_view_course.html', {'courses': courses, 'form': form})
